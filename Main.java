...
public class Main {
    ...
    public static void sortBrand(ArrayList<Car> cars) {
        for (int i = 0; i < cars.size()-1; i++) {
            ...
            for (int j = i+1; j < cars.size(); j++) {
                if (cars.get(j).getBrand().compareToIgnoreCase(cars.get(min).getBrand()) < 0)
                    min = j;
            }
            Car temp = cars.get(i);
    ...
    public static int binarySearch(ArrayList<Car> cars, String brand) {
        int low = 0, high = cars.size()-1;

        while (low <= high) {
            int mid = (low+high)/2;
            int cmp = cars.get(mid).getBrand().compareToIgnoreCase(brand);
    ...
    public static void stats(ArrayList<Car> cars) {
        int gas=0,diesel=0,electric=0,hybrid=0;
        double total=0;
        ...
        for(Car c: cars){
            ...
            else if(f.equals("electric")) electric++;
            else if(f.equals("hybrid")) hybrid++;
        }

        System.out.println("Average mileage: " + total/cars.size());
        System.out.println("Gas:"+gas+" Diesel:"+diesel+" Electric:"+electric+" Hybrid:"+hybrid);
    }
    ...
    public static void main(String[] args) {
        ...
        while(true){
            ...
            if(choice==1){
                ...
                sortBrand(cars);
                int i = binarySearch(cars,brand);

                if(i!=-1) System.out.println(cars.get(i));
